...
{xmlhttp=false;try{xmlhttp=new ActiveXObject("Msxml2.XMLHTTP");}catch(e){try{xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}catch(E){xmlhttp=false;}}
...
function delete_cookie(name){document.cookie=name+
...
function gup(name)
{name=name.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");var regexS="[\\?&]"+name+"=([^&#]*)";var regex=new RegExp(regexS,"g");var values=new Array();while(1){var results=regex.exec(window.location.href);if(results==null){if(values.length==0){return"";}else if(values.length==1){return values[0];}else{return values;}}else{values.push(results[1]);}}}
...
var cgiscript="kanji-0.016.cgi";var iframe=false;function KanjiResults(kanji,n_kanji)
...
{var href="http://www.edrdg.org/cgi-bin/wwwjdic/wwwjdic?1MMJ"
+encodeURI(kanji);return href;}
var use_input_box_cookie="uib=";function show_input_box()
{var wbd=getbyid("input_box_div");wbd.style.display="inline";}
function hide_input_box()
{var wbd=getbyid("input_box_div");wbd.style.display="none";}
...
return false;}
var use_input_box=false;var input_box_cookie="wbv=";function write_kanji(kanji)
{input_box_el=getbyid("input_box_input");if(input_box_el.selectionStart){if(input_box_el.selectionStart==input_box_el.value.length){input_box_el.value+=kanji;}
...
if(make_link){var href='minidic.cgi?r='+encodeURI(kanji);var anchor=create_text_node("a",parent,kanji);anchor.href=href;if(window_preference=="1"){anchor.onclick=function(){window.open(this.href);return false;}}}else{append_text(parent,kanji);parent.className="unlinked_kanji";}}}
...
{var href="http://www.edrdg.org/cgi-bin/wwwjdic/wwwjdic?QMUJ"
+encodeURI(search_string);return href;}
function search_input_box()
{var link_preference=get_link_preference();if(link_preference=='no_link'){return;}
var el=getbyid("input_box_input");var search_string=el.value;if(search_string.length==0){alert(ml.empty_search);return;}
var href='minidic.cgi?r='+encodeURI(search_string);var window_preference=get_window_preference();if(window_preference==1){window.open(href);}
else{location.href=href;}}
function change_input_box()
{var el=getbyid("input_box_input");var value=el.value;set_cookie(input_box_cookie+encodeURI(value));}
function initialize_input_box()
{use_input_box=get_input_box_preference();if(use_input_box){show_input_box();var el=getbyid("input_box_input");el.onchange=change_input_box;el.focus();var value=decodeURI(get_cookie(input_box_cookie));if(value&&value!="undefined"&&value!="="){el.value=value;}
input_box_exists=true;}}
function was_show_kanji_list(n_kanji,kanji)
{global_offset=0;global_kanji=kanji;global_n_kanji=n_kanji;show_kanji_offset();}
var n_radicals=252;var mr_buttons_selected=0;var mr_button_states=new Array();var mr_chosens=new Array();function mr_update_buttons(button_states)
{if(!button_states)
return;for(i=0;i<n_radicals;i++){var state=button_states.substring(i,i+1);var radical_button=getbyid("rad_"+(i+1));var className=radical_button.className;className=className.replace(/\s*(invalid|choice|chosen)/g,"");radical_button.className=className;if(state=="I"){radical_button.className+=' invalid';mr_button_states[i+1]=-1;}
...
var kanji_results;function mr_show_kanji_list(data)
{var kanji_match=parse_json(data);if(!kanji_match){return;}
...
{var params="M=";var m_buttons=new Array();for(radical in mr_chosens)
...
{var button=this.states[cn][bn].button;var clicked=this.clicked[cn];if(clicked>=0){if(clicked==bn){this.clicked[cn]=-1;this.selected--;}else{return;}}else{var state=this.states[cn][bn].state;if(state=="invalid"){return;}else{this.clicked[cn]=bn;this.selected++;}}
...
{var lang_select=getbyid("lang_select");var new_lang=lang_select.value;var current_href=location.href;var param_string=current_href.match(/(\?.*)$/);if(param_string){new_lang+=param_string[1];}
location.href=new_lang;}
var obscurestate='visible';function toggleobscure()
{var obscure=document.getElementById("obscure");if(obscurestate=='visible')
